...
// Legacy decoder (older QR codes encoded with '%' included in alphabet)
const LEGACY_ALPHABET =
  "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_.!~*'()$%+,;=:@?" as const;

export function fromBase80LegacyPercent(text: string): Uint8Array {
  if (!text) return new Uint8Array(0);

  const BASE_LEG = 80;
  // leading zeros
  let zeros = 0;
  while (zeros < text.length && text.charAt(zeros) === LEGACY_ALPHABET[0]) zeros += 1;

  const charToVal: Record<string, number> = Object.create(null);
  for (let i = 0; i < LEGACY_ALPHABET.length; i += 1) {
    charToVal[LEGACY_ALPHABET[i] as unknown as string] = i;
  }
  ...
  for (let i = zeros; i < text.length; i += 1) {
    ...
    if (v === undefined) throw new Error(`Invalid Base80 character: '${ch}'`);
    vals.push(v);
  }
  ...
  while (vals.length > 0) {
    ...
    const next: number[] = [];
    for (let i = 0; i < vals.length; i += 1) {
      const acc = remainder * BASE_LEG + vals[i]!;
      const q = Math.floor(acc / 256);
      remainder = acc % 256;
      if (next.length > 0 || q > 0) next.push(q);
    }
